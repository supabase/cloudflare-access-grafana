...
func VerifyToken(next http.Handler, tokenVerifier *oidc.IDTokenVerifier, cfg *Config) http.Handler {
	fn := func(w http.ResponseWriter, r *http.Request) {
		...
		if accessJWT == "" {
			...
			_, _ = w.Write([]byte("No token on the request"))
			return
		}
		...
		if err != nil {
			w.WriteHeader(http.StatusUnauthorized)
			_, _ = w.Write([]byte(fmt.Sprintf("Invalid token: %s", err.Error())))
			return
		}
		...
		if err := token.Claims(&claims); err != nil {
			w.WriteHeader(http.StatusUnauthorized)
			_, _ = w.Write([]byte(fmt.Sprintf("Invalid claims in token: %s", err.Error())))
		}
